...
! ------------------Input Paramters - Pranav ----------------------------------------------------------- 
  ...
  character(len=19), parameter :: edir = 'turbprod'
...
!-------------------Input parameters -------------------------------------------------------------------
  ...
  real    ( kind = 8 ) :: dumdx,dumdy,dumdz,dvmdx,dvmdy,dvmdz,dwmdx,dwmdy,dwmdz
  ...
  real    ( kind = 8 ),allocatable, dimension(:,:,:) :: stretch_ver, prod, int_prod
...
! Allocate 5p data
  ...
  allocate(prod(nxx,nyy,nkrr),int_prod(nxx,nyy,nkrr))
...
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        DO i=2,NXX-1
          ...
          dumdy = (0.25*( umean(i,j,k)+umean(i,j+1,k)+umean(i-1,j+1,k)+umean(i-1,j,k) ) - &
             0.25*( umean(i,j,k)+umean(i,j-1,k)+umean(i-1,j-1,k)+umean(i-1,j,k) ) )/dy     
          dumdz = (0.25*( umean(i,j,k)+umean(i,j,k+1)+umean(i-1,j,k+1)+umean(i-1,j,k) ) - &
             0.25*( umean(i,j,k)+umean(i,j,k-1)+umean(i-1,j,k-1)+umean(i-1,j,k) ) )/dz

          dvmdx = (0.25*( vmean(i,j,k)+vmean(i,j-1,k)+vmean(i+1,j-1,k)+vmean(i+1,j,k) ) - &
              0.25*( vmean(i,j,k)+vmean(i,j-1,k)+vmean(i-1,j-1,k)+vmean(i-1,j,k) ) )/dx
          dvmdz = (0.25*( vmean(i,j,k)+vmean(i,j-1,k)+vmean(i,j-1,k+1)+vmean(i,j,k+1) ) - &
              0.25*( vmean(i,j,k)+vmean(i,j-1,k)+vmean(i,j-1,k-1)+vmean(i,j,k-1) ) )/dz  

          dwmdx = (0.25*( wmean(i,j,k)+wmean(i,j,k-1)+wmean(i+1,j,k-1)+wmean(i+1,j,k) ) - &
             0.25*( wmean(i,j,k)+wmean(i,j,k-1)+wmean(i-1,j,k-1)+wmean(i-1,j,k) ) )/dx
          dwmdy = (0.25*( wmean(i,j,k)+wmean(i,j+1,k-1)+wmean(i,j+1,k)+wmean(i,j,k-1) ) - &
             0.25*( wmean(i,j,k)+wmean(i,j-1,k-1)+wmean(i,j-1,k)+wmean(i,j,k-1) ) )/dy   
         
          Suw = 0.5*(dumdz + dwmdx)
          Svw = 0.5*(dvmdz + dwmdy)
          Suv = 0.5*(dumdy + dvmdx)

          prod(i,j,k)   = -( u1(i,j,k)*v1(i,j,k)*Suv + u1(i,j,k)*w1(i,j,k)*Suw + v1(i,j,k)*w1(i,j,k)*Svw )
...
! End looping over 5p files 
    ...
    int_prod(:,:,:) =  int_prod(:,:,:)/TOTALT  

    CALL SYSTEM('mkdir ' // trim(edir) )
      
    WRITE(*,*)" prod CALCULATED"

!    filename  = trim(edir)  // '/Prod.5P'
!    call  write_5p(filename,nxx,nyy,nkrr,int_prod,T_INI,TIME1)

    filename  = trim(edir)  // '/Integ_Prod.dat'
    call integxy(filename,int_prod,x5p,y5p,z5p,nxx,nyy,nkrr)
